...
;control FXNAME "Select target EQ effect" choice (("Graphic" "Graphic EQ")
                                                      ("FilterCurve" "Filter Curve EQ")) 0

;control INFILE "Equalization XML file" file "" "*default*/EQCurves.xml" (("XML file" (xml XML))
                        ("All files" ("")))  "open,exists"

$control OVERWRITE "If output text file exists" choice (("Append" "Append number")
                                                            ("Overwrite" "Overwrite")
                                                            ("Error" "Error")) 0
...
(defun process-xml ()
  (let ((fp-in (open INFILE :direction :input))
    ...
    (unless fp-in
      (throw 'err
          (format nil "Error.~%Unable to open file~%~s" INFILE)))
  ...
  (let ((outfile (get-out-file-name (get-path INFILE) curvename))
    ...
    (unless fp-out
      (throw 'err (format nil
                          "Error.~%File cannot be written:~%\"~a.txt\""
                          curvename)))
